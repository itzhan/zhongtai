...
页面级之外还有**字段脱敏**（`src/lib/mask.ts`）：生产看不到卖价、销售看不到进价、财务看不到卡号 CVV、销售财务看不到代理密码。脱敏在 API 层做，响应体里直接是 `null`；仪表盘更进一步——销售拿到的 JSON 里根本不存在 `cost` / `profit` 这两个 key。
