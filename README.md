...
```
收入 = Σ 项目收入流水 (FinanceEntry kind=income)
成本 = Σ 项目成本流水 (FinanceEntry kind=cost)
利润 = 收入 − 成本
```

项目详情里直接记账；成本流水与「采购记录」页同源。台子 NewAPI/Sub2API 消耗接口本期为占位，**暂不计入利润**。旧的台子卖价合计 / 供货进货 / 资源 Purchase 表不再参与项目利润。
